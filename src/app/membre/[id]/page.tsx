          ...
          <div className="relative" ref={settingsMenuRef}>
            ...
            {showSettingsMenu && (
              <div className="absolute right-0 top-10 z-[60] w-52 overflow-hidden rounded-2xl border border-line bg-paper shadow-2xl">
                ...
                <button
                  onClick={() => { toggleDark(); setShowSettingsMenu(false); }}
      ...
      {wantToRead.length > 0 && (
        <section className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {wantToRead.map((b) => (
              <Link key={b.id} href={"/livre/" + b.id} className="group relative flex flex-col gap-1.5">
          ...
          {!isOwn && (
            <p className="text-[11px] text-muted">
              Ces livres sont dans la liste d&apos;envies de {profile.display_name}.
            </p>
          )}
        </section>
      )}
